  ...
  for _ in 0..15 {
    skies.push(Sky {
      screen_position: Vector2::new(
        ...
        rand::thread_rng().gen::<f64>() * 0.36,
      ),
      steepness: rand::thread_rng().gen_range(0.1, 0.2),
    });
  }
  let mut state = State {
    path: Path {
...
fn main() {
  ...
  let start_playing_callback = {
    ...
    move || {
      ...
      if match game.menu_state {
        MenuState::MainMenu | MenuState::MainMenuAppearing(_) => true,
        _ => false,
      } {
  ...
  let back_to_menu_callback = {
    ...
    move || {
      let mut game = game.borrow_mut();
      if match game.menu_state {
        MenuState::GameOver | MenuState::GameOverAppearing(_) => true,
        _ => false,
      } {
...

  {
    ...
    let mousemove_callback = move |x: f64, y: f64| {
      ...
      if limited_angle > limit || limited_angle < -limit {
        offset = Rotation2::new(
          -limit * offset[0].signum() * game.most_recent_movement_direction - absolute_angle,
        ) * offset;
      } else {
        game.most_recent_movement_direction = if location[1] < player_center[1] {
          -1.0
        } else {
          1.0
        };
    ...
    if duration_to_simulate > 0.0 {
      ...
      match game.menu_state {
        MenuState::MainMenuAppearing(progress) => {
          ...
          let new_progress =
            progress + duration_to_simulate / auto_constant("main_menu_fade_in", 4.0);
          game.menu_state = if new_progress > 1.0 {
            MenuState::MainMenu
          } else {
            MenuState::MainMenuAppearing(new_progress)
          };
        }
        MenuState::MainMenu => {
          js! {
        ...
        MenuState::MainMenuDisappearing(progress) => {
          ...
          game.menu_state = if new_progress > 1.0 {
            ...
            MenuState::MainMenuDisappearing(new_progress)
          };
        ...
        MenuState::Playing => {
          ...
          draw = true;
          if game.state.now > auto_constant("game_duration", 10.0 * 60.0) {
            game.menu_state = MenuState::GameEnding(0.0);
          }
        }
        MenuState::GameEnding(progress) => {
          ...
          draw = true;
          let new_progress =
            progress + duration_to_simulate / auto_constant("game_fade_out_duration", 3.0);
          game.menu_state = if new_progress > 1.0 {
            MenuState::GameOverAppearing(0.0)
          } else {
            MenuState::GameEnding(new_progress)
          };
        }
        MenuState::GameOverAppearing(progress) => {
          ...
          let new_progress =
            progress + duration_to_simulate / auto_constant("game_over_screen_fade_in", 6.0);
          game.menu_state = if new_progress > 1.0 {
            MenuState::GameOver
          } else {
            MenuState::GameOverAppearing(new_progress)
          };
        }
        MenuState::GameOver => {
          js! {
