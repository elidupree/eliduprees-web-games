...
impl LowpassFilterState {
  pub fn new(order: usize) -> Self {
    LowpassFilterState(vec![0.0; order])
  }
  pub fn apply(&mut self, mut sample: f64, cutoff_frequency: f64, duration: f64) -> f64 {
    let decay_constant =
      TURN * cutoff_frequency / (2f64.powf(1.0 / self.0.len() as f64) - 1.0).sqrt();
...
impl HighpassFilterState {
  pub fn new(order: usize) -> Self {
    HighpassFilterState(vec![0.0; order])
  }
  pub fn apply(&mut self, mut sample: f64, cutoff_frequency: f64, duration: f64) -> f64 {
    let decay_constant =
      TURN * cutoff_frequency * (2f64.powf(1.0 / self.0.len() as f64) - 1.0).sqrt();
...
pub fn maybe_batch<T, F: FnMut(usize, &[T])>(
  samples: &[T],
  constants: &RenderingStateConstants,
  mut batch_function: F,
) {
  if samples.len() % constants.samples_per_illustrated == 0
    || samples.len() == constants.num_samples
  {
    let batch_start =
      ((samples.len() - 1) / constants.samples_per_illustrated) * constants.samples_per_illustrated;
...
impl RenderedSamples {
  pub fn push(&mut self, value: f64, constants: &RenderingStateConstants) {
    ...
    maybe_batch(&self.samples, constants, |batch_start, rendered_slice| {
      ...
      let root_mean_square = (rendered_slice
        .iter()
        .map(|sample| {
        ...
        .sum::<f64>()
        / rendered_slice.len() as f64)
        .sqrt();

      illustration.lines.push(IllustrationLine {
        range: [0.5 - root_mean_square, 0.5 + root_mean_square],
        clipping: rendered_slice.iter().any(|value| value.abs() > 1.0),
      });
...
impl WaveformRenderingState {
  fn next_sample(
    ...
    let result = match definition.clone() {
      ...
      Waveform::PinkNoise => do_pink_noise(
        &mut self.values,
        1.0,
        10.0 / constants.sample_rate as f64,
        &mut self.generator,
      ),
      Waveform::BrownNoise => do_white_noise(
        &mut self.value,
        20.0 / constants.sample_rate as f64,
        &mut self.generator,
      ),
      Waveform::PitchedWhite => do_white_noise(&mut self.value, fraction, &mut self.generator),
      Waveform::PitchedPink => do_pink_noise(
        &mut self.values,
        fraction,
        min(fraction * 0.25, 10.0 / constants.sample_rate as f64),
        &mut self.generator,
      ),
      Waveform::Experimental => {
        let mut sample = do_pink_noise(
          &mut self.values,
          1.0,
          10.0 / constants.sample_rate as f64,
          &mut self.generator,
        ...
        sample = self
          .lowpass_state
          .apply(sample, frequency, constants.sample_duration);
        sample = self
          .highpass_state
          .apply(sample, frequency, constants.sample_duration);
        sample * 4.0
...
impl SignalEffectRenderingState {
  fn next_sample<T: UserNumberType>(
    &mut self,
    definition: &SignalEffect<T>,
    index: usize,
    sample_time: f64,
    smooth: bool,
    constants: &RenderingStateConstants,
    ...
    match definition.clone() {
      SignalEffect::Jump { time, size } => {
        ...
        } else if sample_time > time.rendered {
          size.rendered
        } else {
          0.0
        }
      ...
      SignalEffect::Slide {
        ...
        if smooth {
          duration = max(duration, SMOOTH_TIME);
        }
        if sample_time <= start.rendered {
          0.0
        } else if sample_time >= start.rendered + duration {
          size.rendered
        } else {
      ...
      SignalEffect::Oscillation {
        ...
        size.rendered
          * self
            .waveform
            .next_sample(&waveform, index, sample_time, frequency, phase, constants)
      }
    }
  }
}
...
impl SignalRenderingState {
  fn next_sample<T: UserNumberType>(
    ...
    definition.initial_value.rendered
      + self
        .effects
        .iter_mut()
        .zip(definition.effects.iter())
        .map(|(rendering, definition)| {
          rendering.next_sample(definition, index, time, smooth, constants)
        })
        .sum::<f64>()
  }
}
...
impl RenderingState {
  pub fn sample_signal<Identity: SignalIdentity>(
    &mut self,
    ...
    let sample = rendering.next_sample(
      Identity::definition_getter().get(&definition.signals),
      index,
      time,
      smooth,
      &self.constants,
  ...
  pub fn new(sound: &SoundDefinition) -> RenderingState {
    let num_samples = (min(MAX_RENDER_LENGTH, sound.rendering_duration())
    ...
    for _ in 0..(if sound.enabled::<Harmonics>() {
      max(1.0, min(100.0, sound.signals.harmonics.range()[1].ceil())) as usize
    } else {
      1
    }) {
  ...
  pub fn next_waveform_sample(&mut self, sound: &SoundDefinition) -> f64 {
    ...
    let mut result = 0.0;
    let harmonics = if sound.enabled::<Harmonics>() {
      max(1.0, self.sample_signal::<Harmonics>(sound, true))
    } else {
      1.0
    };
    ...
    for (harmonic_index, waveform) in self.harmonics.iter_mut().enumerate() {
      ...
      if sound.enabled::<WaveformSkew>() {
        ...
        harmonic_phase = if harmonic_phase < skew {
          harmonic_phase * 0.5 / skew
        } else {
          0.5 + (harmonic_phase - skew) * 0.5 / (1.0 - skew)
        };
      }
      let this_sample = waveform.next_sample(
        &sound.waveform,
        index,
        time,
        frequency * harmonic,
  ...
  fn single_step(&mut self, sound: &SoundDefinition) {
    ...
    if sound.enabled::<Chorus>() {
      ...
      if voices > 0.0 {
        for voice in 0..voices.ceil() as usize {
          let fraction = if voices >= (voice + 1) as f64 {
            1.0
          } else {
            (voices - voice as f64).sqrt()
          };
          ...
          let offset =
            ((time * oscillator_speed + initial_phase).sin() - 1.0) * oscillator_amplitude;
          ...
          sample += self
            ...
            .volume
            .rendered_after
            .resample(time + offset, &self.constants)
            * fraction;
        }
        sample /= voices + 1.0;
      }
    ...
    if sound.enabled::<LogFlangerFrequency>() {
      let flanger_frequency = self
        .sample_signal::<LogFlangerFrequency>(sound, true)
        .exp2();
      let flanger_offset = 1.0 / flanger_frequency;
      sample += previous_getter
        .get(&self.signals)
        .rendered_after
        .resample(time - flanger_offset, &self.constants);
      sample += self
        .signals
        .get::<LogFlangerFrequency>()
        .rendered_after
        .resample(time - flanger_offset, &self.constants);
      sample /= 2.0;
      self
        .signals
        .log_flanger_frequency
        .rendered_after
        .push(sample, &self.constants);
    }

    if sound.enabled::<LogLowpassFilterCutoff>() {
      let lowpass_filter_frequency = self
        .sample_signal::<LogLowpassFilterCutoff>(sound, false)
        .exp2();
      sample = self.lowpass_state.apply(
        sample,
        lowpass_filter_frequency,
        self.constants.sample_duration,
      );
      self
        .signals
    ...
    if sound.enabled::<LogHighpassFilterCutoff>() {
      let highpass_filter_frequency = self
        .sample_signal::<LogHighpassFilterCutoff>(sound, false)
        .exp2();
      sample = self.highpass_state.apply(
        sample,
    ...
    if sound.enabled::<BitcrushResolutionBits>() {
      let bits = max(
        1.0,
        self.sample_signal::<BitcrushResolutionBits>(sound, false),
      );
      ...
      sample = if sample_fraction.abs() > 0.25 * (2.0 - bits_fraction) {
        sample_increments_rounded + sample_fraction.signum() * 0.5
      } else {
        sample_increments_rounded
      } * increment
        - 1.0;
      self
        .signals
        .bitcrush_resolution_bits
        .rendered_after
        .push(sample, &self.constants);
    }
    ...
    if sound.enabled::<LogBitcrushFrequency>() {
      ...
      self
        .signals
        .log_bitcrush_frequency
        .rendered_after
        .push(sample, &self.constants);

      let bitcrush_frequency = self
        .sample_signal::<LogBitcrushFrequency>(sound, false)
        .exp2();
