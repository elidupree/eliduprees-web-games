...
pub trait UserNumberType:
  'static + Clone + PartialEq + Serialize + DeserializeOwned + Debug + Default
{
  type DifferenceType: UserNumberType;
  fn render(&self, value: &str) -> Option<f64>;
  fn approximate_from_rendered(&self, rendered: f64) -> String;
  fn unit_name(&self) -> &'static str;
  fn currently_used(_state: &State) -> Self {
    Self::default()
  }
}
...
impl<T: UserNumberType> UserNumber<T> {
  ...
  pub fn from_rendered(rendered: f64) -> Self {
    ...
    Self::new(
      value_type.clone(),
      value_type.approximate_from_rendered(rendered),
    )
    .unwrap()
  }
}
...
pub enum SignalEffect<T: UserNumberType> {
  Jump {
    time: UserTime,
    size: UserNumber<T::DifferenceType>,
  },
  Slide {
    start: UserTime,
    duration: UserTime,
...
pub trait SignalIdentityGetters {
  ...
  type DefinitionGetterBase: Clone
    + 'static
    + GetterBase<From = Signals, To = Signal<Self::NumberType>>;
  type RenderingGetterBase: Clone
    + 'static
    + GetterBase<From = SignalsRenderingState, To = SignalRenderingState>;
...
pub trait SignalIdentity: SignalIdentityGetters {
  fn info() -> SignalInfo;
  fn applicable(_sound: &SoundDefinition) -> bool {
    true
  }
}
...
impl Signals {
  pub fn get<Identity: SignalIdentity>(&self) -> &Signal<Identity::NumberType> {
    Identity::definition_getter().get(self)
  }
  pub fn get_mut<Identity: SignalIdentity>(&mut self) -> &mut Signal<Identity::NumberType> {
    Identity::definition_getter().get_mut(self)
  }
}
impl SignalsRenderingState {
  pub fn get<Identity: SignalIdentity>(&self) -> &SignalRenderingState {
    Identity::rendering_getter().get(self)
  }
  pub fn get_mut<Identity: SignalIdentity>(&mut self) -> &mut SignalRenderingState {
    Identity::rendering_getter().get_mut(self)
  }
}
...
impl SignalIdentity for LogFrequency {
  ...
  fn applicable(sound: &SoundDefinition) -> bool {
...
impl<T: UserNumberType> SignalEffect<T> {
  ...
  pub fn draw_through_time(&self) -> f64 {
    match self.clone() {
      ...
      SignalEffect::Slide {
        start, duration, ..
      } => start.rendered + duration.rendered + 0.1,
...
impl Envelope {
  pub fn duration(&self) -> f64 {
    self.attack.rendered + self.sustain.rendered + self.decay.rendered
  }
  pub fn sample(&self, time: f64) -> f64 {
    if time < self.attack.rendered {
      return time / self.attack.rendered;
    }
    if time < self.attack.rendered + self.sustain.rendered {
      return 1.0;
    }
    if time < self.attack.rendered + self.sustain.rendered + self.decay.rendered {
      return (self.attack.rendered + self.sustain.rendered + self.decay.rendered - time)
        / self.decay.rendered;
    }
    0.0
  }
...
impl SoundDefinition {
  ...
  pub fn enabled<Identity: SignalIdentity>(&self) -> bool {
    Identity::applicable(self)
      && (Identity::definition_getter().get(&self.signals).enabled || !Identity::info().can_disable)
  }
}
...
impl Default for SoundDefinition {
  fn default() -> Self {
    SoundDefinition {
      envelope: Envelope {
        attack: UserNumber::from_rendered(0.1),
        sustain: UserNumber::from_rendered(0.5),
        decay: UserNumber::from_rendered(0.5),
      },
