...
pub fn input_callback_gotten<T, U, F, G: GetterBase<From = State, To = T>>(
...
where
  F: Fn(&mut T, U),
{
  input_callback(state, move |state, arg| {
    (callback)(getter.get_mut(state), arg)
  })
}

pub fn input_callback_gotten_nullary<T, F, G: GetterBase<From = State, To = T>>(
  state: &Rc<RefCell<State>>,
  getter: Getter<G>,
  callback: F,
) -> impl (Fn())
...
>(
  ...
  let mut result = IllustrationCanvas::new(
    state.clone(),
