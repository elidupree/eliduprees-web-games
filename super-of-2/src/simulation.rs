...
pub fn overlapping_tiles(center: Vector2<f64>, size: Vector2<f64>) -> Vec<Vector2<i32>> {
  ...
  (min[0].floor() as i32..max[0].ceil() as i32)
    .flat_map(|x| (min[1].floor() as i32..max[1].ceil() as i32).map(move |y| Vector2::new(x, y)))
    .collect()
}
...
impl State {
      ...
      EntityPosition::Physical(physical) => match physical {
        EntityPhysicalPosition::Map { center } => {
          for tile_location in overlapping_tiles(center, self.entities[&index].size) {
            ...
            if !tile.entities.contains(&index) {
              tile.entities.push(index);
            }
          }
        }
        EntityPhysicalPosition::Inventory { owner, position } => {
          if let Some(&mut Entity {
            inventory: Some(ref mut inventory),
            ..
        ...
        EntityPhysicalPosition::Map { center } => {
          for tile_location in overlapping_tiles(center, self.entities[&index].size) {
            if {
              let tile = self
                .map
                .get_mut(&tile_location)
                .expect("position records should have existed for entity");
  ...
  pub fn move_entity(&mut self, index: Index, new_position: EntityPosition) {
    self.remove_position_records(index);
    {
      ...
      match new_position {
        EntityPosition::Physical(EntityPhysicalPosition::Inventory { .. }) => {
          entity.velocity = Vector2::new(0.0, 0.0)
        }
        _ => (),
      }
    }
    ...
    for index in entities {
      match self.entities[&index].position {
        EntityPosition::Physical(EntityPhysicalPosition::Map { center }) => {
          ...
          self.move_entity(
            index,
            EntityPosition::Physical(EntityPhysicalPosition::Map { center: new_center }),
          );
        }
        _ => (),
      }
  ...
  pub fn screen_to_physical(&self, location: Vector2<f64>) -> EntityPhysicalPosition {
    // TODO: inventories
    EntityPhysicalPosition::Map {
      center: location / self.map_scale,
    }
  }
  ...
  pub fn position_to_screen(&self, position: EntityPosition) -> Option<(Vector2<f64>, f64)> {
    ...
    match position {
      ...
      EntityPosition::BeingDragged {
        physical: EntityPhysicalPosition::Map { .. },
        hovering_at,
      ...
      EntityPosition::BeingDragged {
        physical: EntityPhysicalPosition::Inventory { owner: _, .. },
        hovering_at: _,
      } => None,
    }
  }
  ...
  pub fn entities_at_screen_location(&self, location: Vector2<f64>) -> Vec<Index> {
    ...
    result.extend(
      self
        .entities
        .keys()
        .filter(|&&index| self.entity_overlaps_screen_location(index, location))
        .min()
        .cloned(),
    );
