...
pub trait ActionTrait {
  ...
  */
  fn update(&mut self, game: &mut Game, canceled: bool) -> ActionStatus;
...
macro_rules! action_enum {
  ($($Variant: ident,)*) => {
    ...
    impl ActionTrait for Action {
      fn update(&mut self, game: &mut Game, canceled: bool) -> ActionStatus {
        match self {
          $(Action::$Variant(s) => s.update(game, canceled),)*
        }
      }
...
impl SimpleAction {
  ...
  fn update(
    ...
    game: &mut Game,
    canceled: bool,
    finish: impl FnOnce(&mut Game),
  ) -> ActionStatus {
    let canceled = canceled && !self.finished();
    if canceled {
      ...
      game.player.health -= health_payment;
...
impl ActionTrait for RotateMechanism {
  fn update(&mut self, game: &mut Game, canceled: bool) -> ActionStatus {
    ...
    self.simple.update(game, canceled, |game| {
      game
        .map
        .tiles
        .get_mut(&game.player.position.containing_tile())
        .unwrap()
        .mechanism
        .as_mut()
        .unwrap()
  ...
  fn update(&mut self, game: &mut Game, canceled: bool) -> ActionStatus {
    ...
    self.simple.update(game, canceled, |game| {
      let tile = game
        .map
        .tiles
        .entry(game.player.position.containing_tile())
