...
impl CylindricalPerspective {
  ...
  pub fn ground_screen_drop(&self, fraction_of_visible: f64) -> f64 {
    ...
    let coordinates0 = self.coordinates_on_circle_relative_to_camera(0.0);
    self.horizon_drop
      + (1.0 - self.horizon_drop) * coordinates[1].atan2(coordinates[0])
        / coordinates0[1].atan2(coordinates0[0])
  }

  pub fn screen_drop_to_fraction_of_visible(&self, screen_drop: f64) -> f64 {
    //let coordinates0 = self.coordinates_on_circle_relative_to_camera (0.0);
    if screen_drop < self.horizon_drop {
      return 1.0;
    }
...
pub fn segment_intersection(first: [Vector2; 2], second: [Vector2; 2]) -> Option<Vector2> {
  ...
  fn convert_segment(segment: [Vector2; 2]) -> LineSegment {
    LineSegment {
      from: convert(segment[0]),
      to: convert(segment[1]),
    }
  }
  convert_segment(first)
    .intersection(&convert_segment(second))
    .map(|vector| convert_back(vector))
}
...
impl ScreenMountain {
  ...
  pub fn key_points(&self, visible_radius: f64) -> [Vector2; 3] {
    [
      self.peak,
      if self.peak[0] - self.radius < -visible_radius {
        Vector2::new(-visible_radius, self.sample(-visible_radius).unwrap_or(0.0))
      } else {
        Vector2::new(self.peak[0] - self.radius, 0.0)
      },
      if self.peak[0] + self.radius > visible_radius {
        Vector2::new(visible_radius, self.sample(visible_radius).unwrap_or(0.0))
      } else {
  ...
  pub fn intersection_points(
    ...
    if let Some(intersection) =
      segment_intersection([self.peak, my_points[1]], [other.peak, other_points[1]])
    {
      result.push(intersection);
    }
    if let Some(intersection) =
      segment_intersection([self.peak, my_points[2]], [other.peak, other_points[1]])
    {
      result.push(intersection);
    }
    if let Some(intersection) =
      segment_intersection([self.peak, my_points[1]], [other.peak, other_points[2]])
    {
      result.push(intersection);
    }
    if let Some(intersection) =
      segment_intersection([self.peak, my_points[2]], [other.peak, other_points[2]])
    {
      result.push(intersection);
    }
...
pub fn skyline(visible_radius: f64, mountains: &[ScreenMountain]) -> Vec<Vector2> {
  ...
  points.retain(|point| {
    point[0].abs() <= visible_radius
      && mountains
        .iter()
        .all(|mountain| point[1] > mountain.sample(point[0]).unwrap_or(0.0) - 0.00001)
  });
...
pub fn as_ground(location: Vector2) -> Vector3 {
  Vector3::new(location[0], location[1], 0.0)
}
...
impl Path {
  pub fn closest_components(&self, vertical_position: f64) -> [Option<&Component>; 2] {
    let next = match self
      .components
      .binary_search_by_key(&OrderedFloat(vertical_position), |component| {
        OrderedFloat(component.center[1])
      }) {
      Ok(i) => i,
      Err(i) => i,
    };
    [
      self.components.get(next.wrapping_sub(1)),
      self.components.get(next),
    ]
  }
...
impl State {
  ...
  pub fn fraction_of_visible(&self, location: Vector3) -> f64 {
    (location[1] - self.player.center[1] + self.constants.player_position)
      / self.constants.visible_length
  }
  ...
  pub fn draw_position(&self, location: Vector3) -> Vector2 {
    ...
    let drop = self
      .constants
      .perspective
      .ground_screen_drop(fraction_of_visible);

    Vector2::new(horizontal_distance * scale, drop - location[2] * scale)
  }

  pub fn screen_to_ground(&self, screen_coordinates: Vector2) -> Vector2 {
    let fraction_of_visible = self
      .constants
      .perspective
      .screen_drop_to_fraction_of_visible(screen_coordinates[1]);
    let scale = self.constants.perspective.scale(fraction_of_visible);
    Vector2::new(
      screen_coordinates[0] / scale + self.player.center[0],
      (fraction_of_visible * self.constants.visible_length) + self.player.center[1]
        - self.constants.player_position,
    )
  }
}
