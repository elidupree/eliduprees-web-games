...
pub fn oscillation_frequency_range() -> [f64; 2] {
  [1f64.log2(), 20f64.log2()]
}
...
pub fn random_number_linear<G: Rng, T: UserNumberType>(
  generator: &mut G,
  range: [f64; 2],
) -> UserNumber<T> {
  UserNumber::from_rendered(generator.gen_range(range[0], range[1]))
}
pub fn random_number_logarithmic<G: Rng, T: UserNumberType>(
  generator: &mut G,
  range: [f64; 2],
) -> UserNumber<T> {
...
pub fn random_signal<G: Rng, T: UserNumberType>(
  generator: &mut G,
  duration: f64,
  info: &SignalInfo,
) -> Signal<T> {
...
pub fn random_difference<G: Rng, T: UserNumberType>(
  generator: &mut G,
  info: &SignalInfo,
) -> UserNumber<T> {
  if !info.differences_are_intervals || (generator.gen_range(0, 3) == 0) {
    random_number_linear(
      generator,
...
pub fn random_signal_effect<G: Rng, T: UserNumberType>(
  generator: &mut G,
  duration: f64,
  info: &SignalInfo,
) -> SignalEffect<T> {
...
pub fn random_jump_effect<G: Rng, T: UserNumberType>(
  generator: &mut G,
  duration: f64,
  info: &SignalInfo,
) -> SignalEffect<T> {
...
pub fn random_slide_effect<G: Rng, T: UserNumberType>(
  generator: &mut G,
  duration: f64,
  info: &SignalInfo,
) -> SignalEffect<T> {
...
pub fn random_oscillation_effect<G: Rng, T: UserNumberType>(
  generator: &mut G,
  _duration: f64,
  info: &SignalInfo,
) -> SignalEffect<T> {
...
pub fn random_sound<G: Rng>(generator: &mut G) -> SoundDefinition {
  ...
  impl<'a, G: Rng> SignalVisitor for Visitor<'a, G> {
    fn visit<Identity: SignalIdentity>(&mut self) {
      ...
      *Identity::definition_getter().get_mut(&mut self.0.signals) =
        random_signal(self.1, duration, &Identity::info());
    }
  }
  ...
  for _attempt in 0..90 {
    ...
    if log_frequency_range[0] < info.slider_range[0]
      || log_frequency_range[1] > info.slider_range[1]
    {
  ...
  for attempt in 0..max_attempts {
    ...
    if increase.abs() > 0.001 {
      sound.signals.volume.initial_value =
        UserNumber::from_rendered(sound.signals.volume.initial_value.rendered + increase);
    }
    let waveform_skew_range = sound.signals.waveform_skew.range();
    if max(waveform_skew_range[1].abs(), waveform_skew_range[0].abs()) > 5.0 {
      sound.signals.waveform_skew =
        random_signal(generator, sound.envelope.duration(), &WaveformSkew::info());
    }
    if sound.signals.log_lowpass_filter_cutoff.range()[0] < sound.signals.log_frequency.range()[1] {
      ...
      sound.signals.log_lowpass_filter_cutoff = if last {
        Signal::constant(UserNumber::from_rendered(info.slider_range[1]))
      } else {
        random_signal(generator, sound.envelope.duration(), &info)
      };
    }
    if sound.signals.log_highpass_filter_cutoff.range()[1] > sound.signals.log_frequency.range()[0]
    {
      ...
      sound.signals.log_highpass_filter_cutoff = if last {
        Signal::constant(UserNumber::from_rendered(info.slider_range[0]))
      } else {
        random_signal(generator, sound.envelope.duration(), &info)
      };
    }
    if sound.signals.log_bitcrush_frequency.range()[0] < sound.signals.log_frequency.range()[1] {
      sound.signals.log_bitcrush_frequency = random_signal(
        generator,
        sound.envelope.duration(),
        &LogBitcrushFrequency::info(),
      );
    }
  }
...
fn max_switch_chance(equilibrium_true_chance: f64, current_state: bool) -> f64 {
  if current_state {
    1.0 - equilibrium_true_chance
  } else {
    equilibrium_true_chance
  }
}
...
fn max_absolute_poisson_exchange_chance(lambda: f64, lower_outcome: usize) -> f64 {
  min(
    poisson_chance(lambda, lower_outcome),
    poisson_chance(lambda, lower_outcome + 1),
  ) * 0.5
}
fn max_relative_poisson_exchange_chance(
  lambda: f64,
  lower_outcome: usize,
  from_outcome: usize,
) -> f64 {
...
fn mutate_uniformly_distributed<G: Rng>(
  generator: &mut G,
  range: [f64; 2],
  max_change_size: f64,
  value: f64,
  ...
  let result_range = [value + offset - max_change_size, value + offset];
  ...
  if value < range[0] {
    ...
    generator.gen_range(
      max(range[0], result_range[0]),
      min(range[1], result_range[1]),
    )
  }
}

fn mutate_logarithmically_distributed<G: Rng>(
  generator: &mut G,
  range: [f64; 2],
  relative_max_change_size: f64,
  value: f64,
) -> f64 {
  if value <= range[0] * 0.5 {
    mutate_uniformly_distributed(
      generator,
      range,
      relative_max_change_size * range[0] * 0.5,
      value,
    )
  } else {
    ...
    mutate_uniformly_distributed(
      generator,
      range,
      relative_max_change_size * (range[1] - range[0]),
      value.log2(),
    )
    .exp2()
  }
...
fn mutate_number_uniformly_distributed<G: Rng, T: UserNumberType>(
  generator: &mut G,
  range: [f64; 2],
  max_change_size: f64,
  value: &UserNumber<T>,
) -> UserNumber<T> {
  UserNumber::from_rendered(mutate_uniformly_distributed(
    generator,
    range,
    max_change_size,
    value.rendered,
...
fn mutate_number_logarithmically_distributed<G: Rng, T: UserNumberType>(
  generator: &mut G,
  range: [f64; 2],
  max_change_size: f64,
  value: &UserNumber<T>,
) -> UserNumber<T> {
  UserNumber::from_rendered(mutate_logarithmically_distributed(
    generator,
    range,
    max_change_size,
    value.rendered,
...
impl<'a, G: 'a + Rng> SoundMutator<'a, G> {
  pub fn mutate_number<T: UserNumberType>(&mut self, number: &mut UserNumber<T>, range: [f64; 2]) {
    if self.generator.gen::<f64>() < self.tweak_chance {
      *number = mutate_number_uniformly_distributed(
        self.generator,
        range,
        self.tweak_size * (range[1] - range[0]),
        number,
      );
    }
  }
  pub fn mutate_number_logarithmic<T: UserNumberType>(
    &mut self,
    number: &mut UserNumber<T>,
    range: [f64; 2],
  ) {
    if self.generator.gen::<f64>() < self.tweak_chance {
      *number =
        mutate_number_logarithmically_distributed(self.generator, range, self.tweak_size, number);
    }
  }
  ...
  pub fn mutate_difference<T: UserNumberType>(
    &mut self,
    difference: &mut UserNumber<T>,
    info: &SignalInfo,
  ) {
    self.mutate_number(
      difference,
  ...
  pub fn mutate_signal_effect<T: UserNumberType>(
    &mut self,
    effect: &mut SignalEffect<T>,
    info: &SignalInfo,
  ) {
    match effect {
      ...
      SignalEffect::Oscillation {
        ...
        self.mutate_difference(size, info);
        if self.generator.gen::<f64>() < self.tweak_chance {
          *frequency = mutate_number_uniformly_distributed(
            self.generator,
            oscillation_frequency_range(),
            self.tweak_size,
            frequency,
          );
        }
  ...
  pub fn mutate_signal<T: UserNumberType>(&mut self, signal: &mut Signal<T>, info: &SignalInfo) {
    ...
    if signal.enabled || !info.can_disable {
      ...
      let increase_chance = self.flop_chance
        * max_relative_poisson_exchange_chance(info.average_effects, num_effects, num_effects);
      let roll: f64 = self.generator.gen::<f64>() - increase_chance;
      if roll < 0.0 {
        signal
          .effects
          .push(random_signal_effect(self.generator, self.duration, info));
      } else if num_effects > 0 {
        let reduce_chance = self.flop_chance
          * max_relative_poisson_exchange_chance(
            info.average_effects,
            num_effects - 1,
            num_effects,
          );
        if roll < reduce_chance {
          signal
            .effects
            .remove(self.generator.gen_range(0, num_effects));
        }
      }
      ...
      if self.generator.gen::<f64>() < self.tweak_chance {
        signal.initial_value = mutate_number_uniformly_distributed(
          self.generator,
          info.slider_range,
          (info.slider_range[1] - info.slider_range[0]) * self.tweak_size,
  ...
  pub fn mutate_sound(&mut self, sound: &mut SoundDefinition) {
    ...
    let old_lowpass_badness = if sound.enabled::<LogLowpassFilterCutoff>() {
      max(
        0.0,
        sound.signals.log_frequency.range()[1] - sound.signals.log_lowpass_filter_cutoff.range()[0],
      )
    } else {
      0.0
    };
    let old_highpass_badness = if sound.enabled::<LogHighpassFilterCutoff>() {
      max(
        0.0,
        sound.signals.log_highpass_filter_cutoff.range()[1]
          - sound.signals.log_frequency.range()[0],
      )
    } else {
      0.0
    };
    let old_bitcrush_badness = if sound.enabled::<LogBitcrushFrequency>() {
      max(
        0.0,
        sound.signals.log_frequency.range()[1] - sound.signals.log_bitcrush_frequency.range()[0],
      )
    } else {
      0.0
    };
      ...
      impl<'b, 'a, G: Rng> SignalVisitor for Visitor<'b, 'a, G> {
        fn visit<Identity: SignalIdentity>(&mut self) {
          if Identity::applicable(self.1) {
            self
              .0
              .mutate_signal(self.1.signals.get_mut::<Identity>(), &Identity::info());
          }
        }
      }
    ...
    let lowpass_badness = if sound.enabled::<LogLowpassFilterCutoff>() {
      max(
        0.0,
        sound.signals.log_frequency.range()[1] - sound.signals.log_lowpass_filter_cutoff.range()[0],
      )
    ...
    let highpass_badness = if sound.enabled::<LogHighpassFilterCutoff>() {
      max(
        0.0,
        sound.signals.log_highpass_filter_cutoff.range()[1]
          - sound.signals.log_frequency.range()[0],
    ...
    let bitcrush_badness = if sound.enabled::<LogBitcrushFrequency>() {
      max(
        0.0,
        sound.signals.log_frequency.range()[1] - sound.signals.log_bitcrush_frequency.range()[0],
      )
    ...
    if old_lowpass_badness < 0.01 && lowpass_badness > 0.0 {
      sound.signals.log_lowpass_filter_cutoff.initial_value = UserNumber::from_rendered(
        sound
          .signals
          .log_lowpass_filter_cutoff
          .initial_value
          .rendered
          + lowpass_badness,
      );
    ...
    if old_highpass_badness < 0.01 && highpass_badness > 0.0 {
      sound.signals.log_highpass_filter_cutoff.initial_value = UserNumber::from_rendered(
        sound
          .signals
          .log_highpass_filter_cutoff
          .initial_value
          .rendered
          - highpass_badness,
      );
    ...
    if old_bitcrush_badness < 0.01 && bitcrush_badness > 0.0 {
      sound.signals.log_bitcrush_frequency.initial_value = UserNumber::from_rendered(
        sound.signals.log_bitcrush_frequency.initial_value.rendered + bitcrush_badness,
      );
    }
  }
}
